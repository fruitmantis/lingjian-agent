...
- 普通用户工作区包括：开启新任务（项目找伙伴 / 能力发展）、场景广场、伙伴洞察、资源中心、全部任务与历史任务、任务详情和个人中心。旧 `/enablement` 只作兼容跳转。
- 管理后台包括：后台概览、全量任务、伙伴管理与案例共享、课程/实验资源管理、需求画像、项目机会、运营报表、能力标签、用户管理、模型配置和系统状态。
- 全局视觉保持黑白灰为主、华为红 `#C7000B` 小面积强调；沿用红色图标和“伴飞 Agent”字标，管理后台一致，不恢复品牌蓝。
- 保留成功、警告、错误语义色，不因品牌色改变状态含义。
- 能力发展已实现：伙伴 + 自然语言方向；解释/比较不创建版本，真实修改成功创建新版本。保留 Plan/Run/Version、current/confirmed、三维权限、撤权、幂等与失败恢复；普通用户不使用结构化高级编辑。
...
- 匹配任务归属以 `match_records.owner_user_id` 为准；能力发展归属以 `development_plans.owner_user_id` 为准。列表、详情、重试、归档、恢复和项目机会更新都必须校验所有权；不得退回按前端参数、`created_by` 文本或 UI 隐藏实现隔离。
- 普通用户越权访问其他用户任务或关联数据时返回 404，避免泄露资源是否存在；不得改成返回对方数据或仅在前端拦截。
- 普通用户只读访问启用的伙伴、案例和交付物摘要；伙伴、案例、交付物、文档和 AI 画像写操作仅限管理员。

## 用户管理与认证安全

- 登录页账号申请仅面向公司内部人员；默认仅展示登录，点击“注册”打开申请；申请包含姓名、工号、部门、邮箱、用户名、密码和确认密码，申请说明选填，并由管理员批准或驳回。
...
- 不得删除、清空、重建或替换当前 `.isolation/runtime/dev/app.db`，不得删除/覆盖现有上传文件。legacy 目录和数据库保留且不得修改，legacy 服务保持停止。其他已有数据库、备份和快照同样不得清理。

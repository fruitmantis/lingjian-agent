...
manifest=json.loads((ROOT/'.isolation/evidence/phase-d-stable-start.json').read_text())
...
# The only stable HTTP request allowed by the Phase D instruction.
...
output=ROOT/'artifacts/enablement-phase-d/protection-verification.json';output.write_text(json.dumps(result,ensure_ascii=False,indent=2));print(json.dumps(result,ensure_ascii=False))

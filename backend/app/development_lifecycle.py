...
def recover(startup=False):
    with get_db() as conn:
        conn.execute('BEGIN IMMEDIATE')
        threshold=(datetime.now(timezone.utc)-timedelta(minutes=10)).isoformat()
        rows=conn.execute("SELECT id,plan_id FROM development_runs WHERE status IN ('pending','running') AND (? OR COALESCE(started_at,created_at)<?)",(int(startup),threshold)).fetchall()
        for row in rows:

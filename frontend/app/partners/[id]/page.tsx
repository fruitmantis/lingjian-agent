...
export default function PartnerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="page">
      <div className="page-heading-row"><div><p className="eyebrow">Partner Detail</p><h1>{partner.name}</h1><p className="lead">{partner.intro || "暂无伙伴简介"}</p></div><div className="enablement-actions"><Link href={`/enablement?partner_id=${encodeURIComponent(id)}`} className="secondary-btn">制定发展方案</Link><Link href="/partners" className="secondary-btn">返回伙伴洞察</Link></div></div>

...
async function snap(page:Page,name:string,width:number){await page.addStyleTag({content:'nextjs-portal {display:none}'});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBeTruthy();await expect(page.locator('aside').getByRole('link',{name:'伙伴服务能力发展中心',exact:true})).toBeInViewport();await page.screenshot({path:path.resolve(`../artifacts/enablement-phase-d/${name}-${width}.png`),fullPage:true});}

test.beforeAll(async({request:r})=>{admin=await login(r,'admin1');user=await login(r,'user_a');other=await login(r,'user_b');await mkdir(path.resolve('../artifacts/enablement-phase-d'),{recursive:true});const tags=await ok(await r.get(API+'/development/capabilities',{headers:user}));tag=tags.find((t:{name:string})=>t.name==='数据库').id;
...
test('NFR-01 real HTTP creation P95 returns before mock generation',async({request:r})=>{
 ...
 await writeFile(path.resolve('../artifacts/enablement-phase-d/http-creation-latency.json'),JSON.stringify({sample_count:20,p50_ms:[...durations].sort((a,b)=>a-b)[9],p95_ms:p95,max_ms:Math.max(...durations),environment:'HTTP 8100, SQLite /tmp, loopback OpenAI-compatible mock, serial submissions',real_model_calls:0},null,2));
});

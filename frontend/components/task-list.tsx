  ...
  return (
    <>
      ...
      <section className="card">
        ...
        {loading ? <p>加载中...</p> : items.length === 0 ? <div className="empty-state"><h2>{archived ? "暂无已归档任务" : "暂无任务"}</h2><p>{taskType === "development_plan" ? "发展方案生成尚未开放，当前没有此类型任务。" : admin ? "当前筛选条件下没有任务。" : "前往开启新任务，完成第一次伙伴匹配。"}</p>{!admin && taskType !== "development_plan" && <Link href="/" className="btn-primary-lg">开启新任务</Link>}</div> : (
          <>

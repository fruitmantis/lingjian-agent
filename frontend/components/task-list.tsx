...
type Task = {
  ...
  taskStatus: TaskStatus; lastErrorStage: string | null;
};
  ...
  return (
    <>
      ...
      <section className="card">
        ...
        {loading ? <p>加载中...</p> : items.length === 0 ? <div className="empty-state"><h2>{archived ? "暂无已归档任务" : "暂无任务"}</h2><p>{admin ? "当前筛选条件下没有任务。" : "前往开启新任务，完成第一次伙伴匹配。"}</p>{!admin && <Link href="/" className="btn-primary-lg">开启新任务</Link>}</div> : (
          <>
            <div className="table-wrap"><table className="data-table"><thead><tr><th>需求摘要</th>{admin && <><th>创建人</th><th>部门</th></>}<th>状态</th><th>首选伙伴</th><th>推荐数</th><th>机会完整度</th><th>创建时间</th><th>操作</th></tr></thead><tbody>
              {items.map(item => <tr key={item.id}><td className="task-requirement">{item.requirement}</td>{admin && <><td>{item.ownerName || "-"}</td><td>{item.department || "-"}</td></>}<td><span className={`status-badge task-${item.taskStatus}`} title={errorStageText(item.lastErrorStage)}>{statusLabels[item.taskStatus]}</span></td><td>{item.topPartner}</td><td>{item.partnerCount}</td><td>{item.completenessScore == null ? "-" : `${item.completenessScore}%`}</td><td>{new Date(item.createdAt).toLocaleString("zh-CN")}</td><td><div className="table-actions"><Link href={`/tasks/${item.id}${admin ? "?from=admin" : ""}`} className="secondary-btn">详情</Link>{(item.taskStatus === "partial" || item.taskStatus === "failed") && <button className="secondary-btn" disabled={retryingId === item.id} onClick={() => void retry(item)}>{retryingId === item.id ? "重试中..." : "重试"}</button>}<button className="secondary-btn" onClick={() => void toggleArchive(item)}>{item.archivedAt ? "恢复" : "归档"}</button></div></td></tr>)}
            </tbody></table></div>

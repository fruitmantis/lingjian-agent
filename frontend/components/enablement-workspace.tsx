  ...
  return <div className="page enablement-page">
    ...
    {resourceTab ? <ResourceCatalog/> : <>
      ...
      {context.error ? <ReadError message={context.error} retry={context.retry}/> : !context.data ? <p role="status">正在核验来源上下文…</p> : <div className="enablement-context-grid">
        ...
        <section className="card"><h2>来源上下文</h2>{context.data.project ? <><Link href={`/tasks/${encodeURIComponent(context.data.project.task_id)}`}>返回来源项目任务</Link><h3>项目需求</h3><p className="enablement-prose">{context.data.project.requirement}</p><h3>原匹配风险 / 缺口</h3><span className="enablement-badge">{context.data.project.risk_status}</span><p className="enablement-prose">{context.data.project.risk_notes||"原匹配未提供风险信息"}</p><p className="muted">这是原匹配提示，尚未确认能力短板，也未转化为培训需求。</p></> : null}

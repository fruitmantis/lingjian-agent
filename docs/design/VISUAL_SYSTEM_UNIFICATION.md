...
当前伴飞 Agent 使用本文的共享视觉规则；实际 token 以 `frontend/app/ui-system.css` 和 `globals.css` 为准。品牌、字体和导航已按当前 main 收口，环境以 [README](../../README.md) 为准。

本文保留 2026-09-08 视觉统一的验证记录，测试起点为 `ad0cbf28e3616951f3c87871c0c999bd023d27fb`；结果不代表最新 HEAD 全量重跑。该次没有真实模型调用，不表示当前业务未获授权。
...
按 [README](../../README.md) 使用当前 main 的 3000/8000，保持 PostgreSQL 与测试数据隔离；不要恢复历史测试模型服务或旧工作区。

重点检查目录卡片悬停、输入卡片静止、两类任务入口、Advisor、资源中心、伙伴洞察及后台表格，使用 1366×768 和 1920×1080。宽表仅允许容器内滚动。项目机会筛选已在后续 `502eb29` 改为紧凑多选，详情可展开；不要按旧截图恢复撑高筛选行的原生多选框。
